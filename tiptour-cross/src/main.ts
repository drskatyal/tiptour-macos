...
apiKeySaveButton.addEventListener("click", async () => {
  ...
  // Save first, validate second — running the validator before
  // save was producing false "rejected" toasts when Gemini's
  // /v1beta/models endpoint returned anything other than a plain
  // 200 (rate limit, CORS preflight, intermittent 5xx). The user
  // reported saving a known-good key and seeing it rejected
  // anyway, which is exactly the broken UX we want to avoid.
  // The actual Live WebSocket open will surface any real auth
  // failure with the server's own error message.
  try {
